...
const AuraVideoPlayer = ({ videoSrc, posterSrc, title }: { videoSrc: string, posterSrc?: string, title: string }) => {
  ...
  const [isBuffering, setIsBuffering] = useState(false); // NEW: Tracking buffer state
  ...
  // Reset states when a new movie is selected
  useEffect(() => {
  ...
  const togglePlay = () => {
    if (videoRef.current) {
      if (isPlaying) {
        ...
        setIsBuffering(true); // Instantly show loader when user clicks play
        const playPromise = videoRef.current.play();
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              // Note: setIsBuffering(false) is handled cleanly by the onPlaying event below
            })
            .catch((error) => {
  ...
  const handleSeek = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (videoRef.current) {
      ...
      setProgress(seekTo);
      setIsBuffering(true); // Show loader while seeking
    }
  };
  ...
  return (
    ...
    >
      <video
        ...
        onWaiting={() => setIsBuffering(true)} // Native event: triggers when starved for data
        onPlaying={() => { setIsBuffering(false); setIsPlaying(true); }} // Native event: triggers when data arrives
      ...
      {/* --- NETFLIX-STYLE LOADING SPINNER OVERLAY --- */}
      <AnimatePresence>
        {isBuffering && (
          <motion.div 
            initial={{ opacity: 0 }} 
          ...
          >
            <Loader2 className="w-12 h-12 md:w-16 md:h-16 text-red-600 animate-spin drop-shadow-[0_0_15px_rgba(220,38,38,0.5)]" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent pt-12 pb-4 px-4 transition-opacity duration-300 z-20 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="flex items-center gap-3 mb-2 group/slider pointer-events-auto">
          <input 
            type="range" 
